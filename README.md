...
- **健檢**：`.\check-links.ps1`
  - `-SkipUrls` — 只跑結構檢查（wikilink 解析 + 中英對照完整性 + **引用完整性**），數秒完成。**每次編輯筆記後都該跑**。
    - 引用完整性檢查兩件事：內文每個 `[n]` 標記都要在文末參考清單中存在；同一篇的中英參考條目數必須相等。
    - 存在理由：2026-07-13 引用文章級化壓縮參考清單時未同步重編內文標記，20 篇（中英各 10）的 `[10]`–`[23]` 指向不存在的條目、3 篇英文版刪來源未重編號使指標錯位，潛伏近兩個月——前兩項檢查看不到引用編號。2026-09-04 修復並補上本檢查。
...
5. **`.\check-links.ps1 -SkipUrls`** ——數秒，wikilink、對照表與引用完整性三項全綠才算完。
